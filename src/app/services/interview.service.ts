...
export class InterviewService {
  ...
  private selectRandomQuestions(): string[] {
    const shuffled = [...NATURALIZATION_QUESTIONS].sort(() => Math.random() - 0.5);
  ...
  evaluateAnswerLocally(question: string, userAnswer: string): QuestionEvaluation {
    const expectedAnswers = QUESTION_ANSWERS[question as keyof typeof QUESTION_ANSWERS] || [];
    const userAnswerLower = userAnswer.toLowerCase().trim();
    
    // More flexible matching for naturalization test
    const isCorrect = expectedAnswers.some(expected => {
      const expectedLower = expected.toLowerCase();
      return userAnswerLower.includes(expectedLower) || 
             expectedLower.includes(userAnswerLower) ||
  ...
  private fuzzyMatch(input: string, expected: string): boolean {
    ...
    return matches.length > 0 && matches.length >= Math.min(inputWords.length, expectedWords.length) * 0.6;
  }
}